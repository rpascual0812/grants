...
export class ContractPreparationComponent {
    ...
    ngOnInit() {
        this.setForm();
        this.fetchUser();
        this.reviews = this.project?.reviews.filter((review: any) => review.type == 'contract_preparation');

        if (this.project?.recommendations) {
            this.project?.recommendations.forEach(recommendation => {
                if (recommendation.type == 'contract_preparation') {
    ...
    export() {
        this.projectService.reviews(this.project?.pk, 'contract_preparation').subscribe({
            next: (data: any) => {
                let reviews = '';
