...
@Component({
    selector: 'app-initial-submission',
    templateUrl: './initial-submission.component.html',
    styleUrls: ['./initial-submission.component.scss']
})
export class InitialSubmissionComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.project?.recommendations) {
            this.project?.recommendations.forEach(recommendation => {
                if (recommendation.type == 'initial_submission') {
    ...
    export() {
        this.projectService.reviews(this.project?.pk, 'initial_submission').subscribe({
            next: (data: any) => {
                let reviews = '';
                ...
                _.exportFile('application/docx', 'Inital Submission.docx', reviews);
            },
            error: (err: HttpErrorResponse) => {
                const errorMessage = err?.error?.message ? `message: ${err?.error?.message}` : '';
                const statusCode = err?.status ? `status: ${err?.status}` : '';
