...
@Component({
    selector: 'app-partner-signing',
    templateUrl: './partner-signing.component.html',
    styleUrls: ['./partner-signing.component.scss']
})
export class PartnerSigningComponent {
    ...
    export() {
        this.projectService.reviews(this.project?.pk, 'partner_signing').subscribe({
            next: (data: any) => {
                let reviews = '';
                ...
                _.exportFile('application/docx', 'Partner Signing.docx', reviews);
            },
            error: (err: HttpErrorResponse) => {
                const errorMessage = err?.error?.message ? `message: ${err?.error?.message}` : '';
                const statusCode = err?.status ? `status: ${err?.status}` : '';
