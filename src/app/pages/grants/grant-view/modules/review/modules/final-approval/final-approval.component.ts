...
export class FinalApprovalComponent {
    ...
    ngOnInit() {
        ...
        if (this.project?.recommendations) {
            this.project?.recommendations.forEach(recommendation => {
                if (recommendation.type == 'final_approval') {
    ...
    export() {
        this.projectService.reviews(this.project?.pk, 'final_approval').subscribe({
            next: (data: any) => {
                let reviews = '';
                ...
                _.exportFile('application/docx', 'Final Approval.docx', reviews);
            },
            error: (err: HttpErrorResponse) => {
                const errorMessage = err?.error?.message ? `message: ${err?.error?.message}` : '';
                const statusCode = err?.status ? `status: ${err?.status}` : '';
